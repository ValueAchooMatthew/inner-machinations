...
<div class=" bg-orange-500 flex shadow-lg py-4 pl-2 pr-4 w-full text-gray-100 ">
    <div class="flex justify-center align-middle w-full gap-6 text-4xl">
        ...
        <div class="flex gap-2 font-bold self-center ml-auto mr-auto">
            <button class={automata_selected === Automata.DFA ? "":"text-black"} on:click={()=>{automata_selected = Automata.DFA;}}>
                DFA
            </button>
            <span >|</span>
            <button class={automata_selected === Automata.NFA ? "":"text-black"} on:click={()=>{automata_selected = Automata.NFA;}}>
                NFA
            </button>
        </div>
