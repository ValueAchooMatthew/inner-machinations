...
<div class="flex justify-start">
    <button class="w-12 h-12 z-10 self-center ml-4 mt-4" on:click={()=>{sidebar_open = !sidebar_open;}}>
...
<div class="font-bold h-fit bg-gray-100 flex flex-col text-3xl justify-start px-96 gap-12 mx-auto rounded-[3rem] p-12">
    <form class="self-start" id="alphabetChange" on:submit={handleSubmitAll}>
        <div class="flex justify-center gap-3">
            ...
            <div class="flex flex-col gap-2">
                ...
                {/each}
                <button on:submit={handleSubmitAll} class="bg-orange-500 w-fit h-fit self-center px-2 py-1 rounded-md text-white" form="alphabetChange">
                    Submit All
                </button>
            </div>

            <button class="self-end mb-1" on:click={handleAddingNewCharInput}>
                <svg class="w-8 h-8 " data-slot="icon" aria-hidden="true" fill="none" stroke-width="1.5" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
    ...
    <form class="self-start">
        ...
        <input class="w-6 h-6" on:change={()=>{is_strict_checking = !is_strict_checking}} type="checkbox" name="strict" id = "strict">
    </form>
    <form class="self-start">
        <label for="char">
            Specify default connection character (default: a): 
        </label>
        <input value={default_connection_char} maxlength="1" on:input={handleDefaultCharChange} class="border-black border-2 rounded-md px-2 py-1" type="text" name="char" id="char">
    </form>
</div>