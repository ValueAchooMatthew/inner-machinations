<script lang="ts">
  ...
  import { handleTrash, redo, undo } from "$lib/utils/deletionFuncs";
  import { current_action, list_of_all_elements, list_of_regular_automata_connections, 
  list_of_states, recently_removed_elements, selected_connection_index, start_state_index, start_state_position, state_positions } from "$lib/utils/regularAutomataStores";
...
<nav class="text-center select-none flex flex-col justify-between self-center gap-3 bg-opacity-100 w-32 h-fit border-black border-2 bg-white rounded-md px-2 py-4 mr-0.5 z-10 transition-all duration-300">
  <div class="flex flex-col gap-2">
    ...
    <button class="flex flex-col"
      ...
      New Connection
      <svg
        class="hover:cursor-pointer w-10 self-center"
        data-slot="icon"
        fill="none"
        stroke-width="1.5"
        stroke="currentColor"
    ...
    <button
      ...
      <br />
      <span class=" -mb-3 self-center"> ϵ </span>
      <svg
        class="hover:cursor-pointer w-10 self-center"
        data-slot="icon"
        fill="none"
  ...
  <div class="flex justify-center gap-4">
    <svg class="hover:cursor-pointer w-6"
      on:click={() => {
        clearCursor();
        undo();
      }}
  ...
  <div class="flex justify-center mt-2">
    <svg class="hover:cursor-pointer w-6"
      ...
      <path
        clip-rule="evenodd"
        fill-rule="evenodd"
        d="M16.5 4.478v.227a48.816 48.816 0 0 1 3.878.512.75.75 0 1 1-.256 1.478l-.209-.035-1.005 13.07a3 3 0 0 1-2.991 2.77H8.084a3 3 0 0 1-2.991-2.77L4.087 6.66l-.209.035a.75.75 0 0 1-.256-1.478A48.567 48.567 
    ...
    <svg
      ...
      <path
        clip-rule="evenodd"
        d="M6.672 1.911a1 1 0 10-1.932.518l.259.966a1 1 0 001.932-.518l-.26-.966zM2.429 4.74a1 1 0 10-.517 1.932l.966.259a1 1 0 00.517-1.932l-.966-.26zm8.814-.569a1 1 
            0 00-1.415-1.414l-.707.707a1 1 0 101.415 1.415l.707-.708zm-7.071 7.072l.707-.707A1 1 0 003.465 9.12l-.708.707a1 1 0 001.415 1.415zm3.2-5.171a1 1 0 00-1.3 1.3l4 10a1 1 0 001.823.075l1.38-2.759 3.018 3.02a1 1 0 001.414-1.415l-3.019-3.02 2.76-1.379a1 1 0 00-.076-1.822l-10-4z"
        fill-rule="evenodd">
      </path>
    </svg>
