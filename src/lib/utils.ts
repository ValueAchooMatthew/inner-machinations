...
const drawNode = (context: CanvasRenderingContext2D, index: number, startStatePosition: number, 
    finalStatePositions: Array<number>, node: State) => {
    context.beginPath();
    context.lineWidth = 3;
...
export const draw = (context: CanvasRenderingContext2D, 
    ...
    startStatePosition: number, finalStatePositions: Array<number>, selectedArrowIndex: number | null) => {
    ...
    nodes.forEach((node, index)=>{
        drawNode(context, index, startStatePosition, finalStatePositions, node);
    })
...
export const initalizeNewArrow = (cursor_x_pos: number, cursor_y_pos: number, connections: Array<Arrow>, 
    ...
    const connection: Arrow = {x1_pos: cursor_x_pos, y1_pos: cursor_y_pos, x2_pos: cursor_x_pos, 
        y2_pos: cursor_y_pos, element: "Connection", character: "a"};
